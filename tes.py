...
# key_up: UP arrow -> thrust upward
# key_w/s/a/d: lean forward/backward/left/right (visual only)
...
def _build_terrain():
    """Build terrain vertex grid and draw it directly (no display lists)."""
    global terrain_verts
...
def draw_cylinder(br, tr, h, sl=28, st=3): gluCylinder(gluNewQuadric(), br, tr, h, sl, st)
def draw_sphere(r, sl=28, st=28):          gluSphere(gluNewQuadric(), r, sl, st)
def draw_disk(inner, outer, sl=28):        gluCylinder(gluNewQuadric(), outer, inner, 0.01, sl, 1)
def draw_cone(base, h, sl=28):             draw_cylinder(base, 0.0, h, sl)
...
def draw_interactive_scene():
    glPointSize(3.0)
    glBegin(GL_POINTS)
    mx = (last_click_x - 500) / 6.0
    my = (400 - last_click_y) / 6.0
    ...
    glPushMatrix()
    glRotatef(animation_time * 8, 1, 1, 0)
    glColor3f(0.3, 0.3, 0.32); gluSphere(q, 16.1, 8, 8)
    glPopMatrix()
    glPopMatrix()
...
def draw_eva_nation():
    ...
    glPushMatrix(); glRotatef(-90, 1, 0, 0); draw_cylinder(0.42, 0.38, 2.2); glPopMatrix()
    ...
    glPushMatrix(); glTranslatef(0, 2.2, 0); glRotatef(-90, 1, 0, 0); draw_cone(0.38, 1.10); glPopMatrix()
    glColor3f(1.0, 0.80, 0.90)
    glPushMatrix(); glTranslatef(0, 3.20, 0); draw_sphere(0.08); glPopMatrix()
    glColor3f(0.85, 0.45, 0.70)
    glPushMatrix(); glRotatef(90, 1, 0, 0); draw_disk(0.0, 0.42); glPopMatrix()
    for i, col in enumerate([(0.80, 0.35, 0.60), (0.90, 0.45, 0.70), (0.80, 0.35, 0.60), (0.90, 0.45, 0.70)]):
        glColor3f(*col)
        glPushMatrix(); glRotatef(i * 90, 0, 1, 0)
    ...
    for nx, ny, nz in [(-0.16, 0, 0), (0.16, 0, 0), (0, 0, 0)]:
        ...
        glPushMatrix(); glTranslatef(nx, -0.05, nz)
        glRotatef(90, 1, 0, 0); draw_cylinder(0.10, 0.08, 0.25); glPopMatrix()
    glColor3f(1.0, 0.85, 0.92)
    for side in [-1, 1]:
        glPushMatrix(); glTranslatef(side * 0.40, 1.10, 0)
        glScalef(0.06, 1.20, 0.06); glutSolidCube(1.0); glPopMatrix()
    glColor3f(0.55, 0.15, 0.80)
    glPushMatrix(); glTranslatef(0, 0.05, 0); glRotatef(90, 1, 0, 0)
    draw_cylinder(0.44, 0.44, 0.05, 48); glPopMatrix()

def draw_jf17_thunder():
    glColor3f(0.55, 0.05, 0.05)
    glPushMatrix(); glRotatef(-90, 1, 0, 0); draw_cylinder(0.34, 0.30, 2.0, 6); glPopMatrix()
    glColor3f(0.70, 0.08, 0.08)
    for px, py, pz, sx, sy, sz in [(0, 1.50, 0.33, 0.24, 0.30, 0.05),
                                    (0, 0.90, 0.33, 0.22, 0.28, 0.05),
                                    (0, 0.35, 0.33, 0.22, 0.28, 0.05)]:
    ...
    glPushMatrix(); glTranslatef(0, 2.0, 0); glRotatef(-90, 1, 0, 0); draw_cone(0.30, 0.90, 6); glPopMatrix()
    glColor3f(0.9, 0.3, 0.3)
    glPushMatrix(); glTranslatef(0, 2.82, 0); draw_sphere(0.06); glPopMatrix()
    glColor3f(0.45, 0.04, 0.04)
    glPushMatrix(); glRotatef(90, 1, 0, 0); draw_disk(0.0, 0.34, 6); glPopMatrix()
    ...
    for sx in [-1, 1]:
        ...
        glPushMatrix(); glTranslatef(sx * 0.46, 0.30, 0)
        glRotatef(-90, 1, 0, 0); draw_cylinder(0.10, 0.08, 0.60); glPopMatrix()
    for nx2, nz2 in [(0, 0), (-0.18, 0), (0.18, 0), (0, -0.18), (0, 0.18)]:
        glColor3f(0.30, 0.03, 0.03)
        glPushMatrix(); glTranslatef(nx2, -0.05, nz2)
        glRotatef(90, 1, 0, 0); draw_cylinder(0.075, 0.065, 0.22); glPopMatrix()
    flame = 0.5 + 0.5 * math.sin(animation_time * 8.0)
    glColor3f(1.0, 0.4 + 0.3 * flame, 0.0)
    glPushMatrix(); glTranslatef(0, -0.05, 0)
    glRotatef(90, 1, 0, 0); draw_cone(0.15, 0.5 + 0.4 * flame, 12); glPopMatrix()

def draw_jak_hound():
    glColor3f(0.14, 0.14, 0.16)
    glPushMatrix(); glRotatef(-90, 1, 0, 0); draw_cylinder(0.32, 0.28, 2.10, 8); glPopMatrix()
    glColor3f(0.85, 0.60, 0.10)
    glPushMatrix(); glTranslatef(0, 1.65, 0); glRotatef(-90, 1, 0, 0)
    draw_cylinder(0.305, 0.295, 0.22, 8); glPopMatrix()
    glColor3f(0.05, 0.55, 0.95)
    glPushMatrix(); glTranslatef(0, 1.52, 0.33); draw_sphere(0.09); glPopMatrix()
    glColor3f(0.10, 0.70, 1.0)
    glPushMatrix(); glTranslatef(0, 1.52, 0.30); glRotatef(90, 0, 1, 0)
    draw_cylinder(0.10, 0.10, 0.02, 24); glPopMatrix()
    ...
    for side in [-1, 1]:
        glPushMatrix(); glTranslatef(side * 0.18, 0.80, 0.28)
        glScalef(0.16, 0.55, 0.04); glutSolidCube(1.0); glPopMatrix()
    glColor3f(0.80, 0.58, 0.10)
    for side in [-1, 1]:
        glPushMatrix(); glTranslatef(side * 0.29, 1.00, 0)
        glScalef(0.04, 1.60, 0.04); glutSolidCube(1.0); glPopMatrix()
    glColor3f(0.18, 0.18, 0.20)
    glPushMatrix(); glTranslatef(0, 2.10, 0); glRotatef(-90, 1, 0, 0); draw_cone(0.28, 0.95, 8); glPopMatrix()
    glColor3f(0.80, 0.60, 0.10)
    glPushMatrix(); glTranslatef(0, 2.97, 0); draw_sphere(0.07); glPopMatrix()
    glColor3f(0.12, 0.12, 0.14)
    glPushMatrix(); glRotatef(90, 1, 0, 0); draw_disk(0.0, 0.32, 8); glPopMatrix()
    for i in range(4):
        glColor3f(0.12, 0.12, 0.14)
        glPushMatrix(); glRotatef(i * 90, 0, 1, 0)
    ...
    glPushMatrix(); glTranslatef(0, -0.05, 0); glRotatef(90, 1, 0, 0)
    draw_cylinder(0.20, 0.17, 0.28, 8); glPopMatrix()
    glColor3f(0.08, 0.08, 0.10)
    glPushMatrix(); glTranslatef(0, -0.05, 0); glRotatef(90, 1, 0, 0)
    draw_cylinder(0.15, 0.12, 0.28, 8); glPopMatrix()
    glColor3f(0.75, 0.55, 0.08)
    glPushMatrix(); glTranslatef(0, -0.02, 0); glRotatef(90, 1, 0, 0)
    draw_cylinder(0.21, 0.21, 0.04, 8); glPopMatrix()
...
# ═══════════════════════════════════════════════════════════════════════════
#  TERRAIN DRAW  — drawn directly with glBegin/glEnd each frame
# ═══════════════════════════════════════════════════════════════════════════
def draw_terrain():
...
def draw_target():
    ...
    for ring_r, lw in [(radius + 2.0, 3.0), (radius, 2.2), (radius * 0.5, 2.0)]:
        glColor3f(tr, tg * (pulse * 0.8 + 0.2), 0.0)
        glBegin(GL_LINE_LOOP)
        for s in range(64):
            a = 2 * math.pi * s / 64
            wx = cx + ring_r * math.cos(a)
            wz = cz + ring_r * math.sin(a)
...
def draw_new_target_arrow():
    ...
    fade  = 1.0 - elapsed / 5.0
    alpha = fade * abs(math.sin(animation_time * 8.0))
...
def draw_space_background():
    ...
    glPushMatrix(); glRotatef(planet_rotation_y * 2, 0, 1, 0)
...
def draw_trajectory():
    ...
    G  = diff_gravity(); DR = diff_drag()
    TU = diff_thrust_up()
    FB = diff_fuel_burn()
    MXH = diff_max_vel_horiz(); MXV = diff_max_vel_up()
    pts = [(px, py, pz)]
    for _ in range(500):
        vy += G
        hf = sim_fuel > 0; ho = sim_oxy > 0
        if key_up:
            if hf:  vy += TU; sim_fuel = max(0, sim_fuel - FB)
            elif ho: vy += TU * 0.55; sim_oxy = max(0, sim_oxy - 0.10)
        wx2, wz2 = get_wind(); vx += wx2; vz += wz2
        vx *= DR; vz *= DR
        vx = max(-MXH, min(MXH, vx)); vz = max(-MXH, min(MXH, vz))
        vy = max(-MXV, min(MXV, vy))
        px += vx; py += vy; pz += vz
        gy = terrain_height(px, pz)
        if py <= gy + 0.5:
            py = gy + 0.5; pts.append((px, py, pz)); break
        pts.append((px, py, pz))
    if len(pts) < 2: return
    spd = math.sqrt(vx ** 2 + vy ** 2 + vz ** 2) * 60
    lv  = get_land_vel_max()
    if spd <= lv:               lr, lg, lb = 0.2, 1.0, 0.3
    elif spd <= lv * 1.5:       lr, lg, lb = 1.0, 0.85, 0.0
    else:                       lr, lg, lb = 1.0, 0.2, 0.1
    glBegin(GL_LINE_STRIP)
    for idx, (tx2, ty2, tz2) in enumerate(pts):
        fade = 1.0 - (idx / len(pts)) * 0.7
        glColor3f(lr * fade, lg * fade, lb * fade)
        glVertex3f(tx2, ty2, tz2)
    glEnd()
    ix, iy, iz = pts[-1]; sz2 = 2.0
    glColor3f(lr, lg, lb)
    glBegin(GL_LINES)
    glVertex3f(ix - sz2, iy + 0.15, iz); glVertex3f(ix + sz2, iy + 0.15, iz)
    glVertex3f(ix, iy + 0.15, iz - sz2); glVertex3f(ix, iy + 0.15, iz + sz2)
    glEnd()
...
def draw_hud():
    ...
    on_oxy    = (fuel <= 0.0 and oxygen > 0.0)
    ...
    glBegin(GL_QUADS)
    glVertex2f(110, WIN_H - 45); glVertex2f(385, WIN_H - 45)
    glVertex2f(385, WIN_H - 32); glVertex2f(110, WIN_H - 32)
    glEnd()
    bw = min(fuel / 100.0, 1.0) * 275
    if on_oxy:      glColor3f(0.9 + 0.1 * blink, 0.5 + 0.3 * blink, 0.0)
    elif fuel < 10: glColor3f(1.0, 0.1, 0.1)
    elif fuel < 30: glColor3f(1.0, 0.6, 0.0)
    else:           glColor3f(0.2, 0.9, 0.2)
    glBegin(GL_QUADS)
    glVertex2f(110, WIN_H - 45); glVertex2f(110 + bw, WIN_H - 45)
    glVertex2f(110 + bw, WIN_H - 32); glVertex2f(110, WIN_H - 32)
    glEnd()
    ...
    glBegin(GL_QUADS)
    glVertex2f(110, WIN_H - 75); glVertex2f(385, WIN_H - 75)
    glVertex2f(385, WIN_H - 62); glVertex2f(110, WIN_H - 62)
    glEnd()
    ow = min(oxygen / 100.0, 1.0) * 275
    glColor3f(*(1.0, 0.1, 0.1) if oxygen < 10 else (1.0, 0.75, 0.0) if oxygen < 30 else (0.2, 0.6, 1.0))
    glBegin(GL_QUADS)
    glVertex2f(110, WIN_H - 75); glVertex2f(110 + ow, WIN_H - 75)
    glVertex2f(110 + ow, WIN_H - 62); glVertex2f(110, WIN_H - 62)
    glEnd()
    ...
    if level >= 4:
        ...
        draw_text(20, WIN_H - 215, "L/R:ROTATE  W/A/S/D:LEAN  UP:THRUST", 0.35, 0.35, 0.35)
    else:
        draw_text(20, WIN_H - 195, "L/R:ROTATE  W/A/S/D:LEAN  UP:THRUST", 0.4, 0.4, 0.4)

    # Active keys indicator
    active = []
    if key_up: active.append("^THRUST(OXY)" if on_oxy else "^THRUST")
    if key_w:  active.append("LEAN FWD")
    if key_s:  active.append("LEAN BACK")
    if key_a:  active.append("LEAN LEFT")
    if key_d:  active.append("LEAN RIGHT")
    if active:
    ...
    if elapsed < 5.0:
        ...
        draw_text(WIN_W // 2 - 175, WIN_H // 2 + 60,
                  f"** NEW TARGET! {dist_to_target:.0f}m AWAY - FOLLOW THE BEACON **",
                  bf * fade2, fade2, 0.0)
    ...
    if game_over and landing_msg:
        ...
        glColor3f(0, 0, 0)
        glBegin(GL_QUADS)
        glVertex2f(WIN_W // 2 - 330, WIN_H // 2 - 100); glVertex2f(WIN_W // 2 + 330, WIN_H // 2 - 100)
        glVertex2f(WIN_W // 2 + 330, WIN_H // 2 + 60);  glVertex2f(WIN_W // 2 - 330, WIN_H // 2 + 60)
        glEnd()
...
def draw_minimap():
    ...
    for s in range(32):
        a = 2 * math.pi * s / 32
        glVertex2f(target_x + rad * math.cos(a), target_z + rad * math.sin(a))
    glEnd()

    glColor3f(0.6, 0.6, 0.0)
    glBegin(GL_LINES)
    glVertex2f(lander_x, lander_z); glVertex2f(target_x, target_z)
    glEnd()
    ...
    glVertex2f(lander_x + math.sin(hr) * 18, lander_z + math.cos(hr) * 18)
    glEnd()
    ...
    draw_text(-half + 5, half - 18, "MAP", 0.6, 0.6, 0.8)
    glPopMatrix()
    glMatrixMode(GL_PROJECTION); glPopMatrix()
    glMatrixMode(GL_MODELVIEW)
...
#  - W/A/S/D:    visual tilt only, no strafe movement
# ═══════════════════════════════════════════════════════════════════════════
def update_physics():
    ...
    G  = diff_gravity(); DR = diff_drag()
    TU = diff_thrust_up(); FB = diff_fuel_burn()
    MXH = diff_max_vel_horiz(); MXV = diff_max_vel_up()
    ...
    # UP arrow → vertical thrust only
    if key_up:
        if hf:   vel_y += TU; fuel   = max(0, fuel   - FB)
        elif ho: vel_y += TU * 0.55; oxygen = max(0, oxygen - 0.10)

    # Oxygen backup fuel regen
    if oo and key_up:
        fuel = min(100, fuel + FUEL_REGEN_RATE); oxygen = max(0, oxygen - 0.10)

    # W/A/S/D → visual lean only (tilt the spacecraft, no velocity change)
    any_tilt = False
    if key_w:
        lander_tilt_x = max(-MAX_TILT, lander_tilt_x - TILT_SPEED); any_tilt = True
    if key_s:
        lander_tilt_x = min( MAX_TILT, lander_tilt_x + TILT_SPEED); any_tilt = True
    if key_a:
        lander_tilt_z = min( MAX_TILT, lander_tilt_z + TILT_SPEED); any_tilt = True
    if key_d:
        lander_tilt_z = max(-MAX_TILT, lander_tilt_z - TILT_SPEED); any_tilt = True
    ...
    if fuel <= 0 and oxygen <= 0 and lander_y > thr:
        landing_msg = f"OUT OF PROPELLANT! Score:{score}"
...
def display():
    ...
    if current_state in ["MENU", "OPTIONS"]:
        ...
        gluPerspective(45, 1.25, 0.1, 600.0)
...
def keyboard(key, x, y):
    ...
    if current_state == "GAMEPLAY":
        if k == b'w': key_w = not key_w; game_started = True
        if k == b's': key_s = not key_s; game_started = True
        if k == b'a': key_a = not key_a; game_started = True
        if k == b'd': key_d = not key_d; game_started = True
        if k == b'r': reset_game()
        if k == b'q': full_reset()
        glutPostRedisplay(); return
            ...
            if current_state == "MENU":
                ...
                if 0 <= selected_option < 3:
                    ...
                    if not rocket_selected:
                        ...
                        full_reset(); current_state = "GAMEPLAY"
...
def special_keys(key, x, y):
    global cam_x, cam_y, selected_option, gp_cam_pitch, key_up, game_started, rocket_heading
    if current_state == "GAMEPLAY":
        if   key == GLUT_KEY_UP:    key_up = not key_up; game_started = True
